...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('simu2v2')
        .setDescription('Inicia um simulador 2v2 por times')
        .addStringOption(o => o.setName('versao').setDescription('Versão').setRequired(true))
        .addIntegerOption(o => o.setName('vagas').setDescription('Total de jogadores').setRequired(true).addChoices({name:'4 (2 Times)',value:4},{name:'8 (4 Times)',value:8}))
    ...
    async execute(interaction) {
        ...
        const menu = new StringSelectMenuBuilder()
            .setCustomId('sel_2v2')
            .setPlaceholder('Escolha seu Time e Vaga')
            .addOptions(Array.from({ length: vagas }, (_, i) => {
                const timeNum = Math.floor(i / 2) + 1;
                const slotNum = (i % 2) + 1;
                return new StringSelectMenuOptionBuilder()
                    .setLabel(`Time ${timeNum} - Vaga ${slotNum}`)
                    .setValue(`${i}`);
            }));
        ...
        col.on('collect', async i => {
            ...
            if (slots[vIdx] !== null) return i.reply({ content: '❌ Esta vaga já está ocupada!', ephemeral: true });
        ...
        col.on('end', async (collected, reason) => {
            ...
            for (let i = 0; i < slots.length; i += 4) {
                ...
                const th = await canal.threads.create({ name: `2v2-Time${idx+1}-vs-Time${idx+2}`, type: ChannelType.PrivateThread });
                ...
                sCol.on('collect', async b => {
                    ...
                    const vTime = isA ? tA : tB; 
                    const pTime = isA ? tB : tA;

                    if (parseInt(b.customId.split('_')[1]) === (vagas === 4 ? 0 : 1)) {
